...
def write_fingerprint(vibe_dir: Path) -> None:
    ...
    (local_marker / "known").write_text(
        read_known_fingerprints(local_marker) + f"{project_id}={token}\n",
        encoding="utf-8",
        newline="\n",
    )


def read_known_fingerprints(local_marker: Path) -> str:
...
def check_fingerprint(vibe_dir: Path) -> bool:
    ...
    local_marker = Path.home() / ".vibe-fingerprints"
    known_lines = read_known_fingerprints(local_marker).splitlines()
    project_id = vibe_dir.resolve().as_posix()

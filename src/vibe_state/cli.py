...
def _write_fingerprint(vibe_dir: Path) -> None:
    """Write a fingerprint to mark .vibe/ as locally generated."""
    import hashlib
    import socket

    fp = hashlib.sha256(
        f"{socket.gethostname()}:{Path.home()}".encode()
    ).hexdigest()[:16]
    fp_path = vibe_dir / ".fingerprint"
    fp_path.write_text(f"vibe-state-cli:{fp}\n", encoding="utf-8", newline="\n")


def _check_fingerprint(vibe_dir: Path) -> bool:
    """Check if .vibe/ was generated on this machine. Returns True if safe."""
    import hashlib
    import socket

    fp_path = vibe_dir / ".fingerprint"
    if not fp_path.exists():
        return False  # No fingerprint = not generated by vibe

    expected = hashlib.sha256(
        f"{socket.gethostname()}:{Path.home()}".encode()
    ).hexdigest()[:16]
    content = fp_path.read_text(encoding="utf-8").strip()
    return content == f"vibe-state-cli:{expected}"
